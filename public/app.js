...
function setLang(code) {
  ...
  // 用的是自动名(没手动改过)→ 跟随新语言重新生成并广播,让名字始终是当前界面语言
  if (!localStorage.deviceName) {
    myName = defaultName(myId);
    if (ws && ws.readyState === 1) ws.send(JSON.stringify({ type: 'rename', name: myName, hue: myHue != null ? myHue : undefined }));
    const sa = document.getElementById('side-avatar'); if (sa) sa.title = myName;
  }
...
// 持久设备指纹(localStorage,跨会话/跨标签稳定):与每标签临时的 deviceId 正交,用于"收藏/信任"和 PIN 免码。
let myFp = localStorage.deviceFp;
if (!myFp) { myFp = 'f' + Math.random().toString(36).slice(2, 12); localStorage.deviceFp = myFp; }
const myKind = /iPhone|iPad|Android/.test(navigator.userAgent) ? 'mobile' : 'desktop';
// 信任设备集合(按持久指纹 fp,localStorage 持久):信任的设备私聊发文件免 PIN,且花名册置顶+加星
let trusted = new Set(); try { trusted = new Set(JSON.parse(localStorage.trustedFps || '[]')); } catch {}
const isTrusted = fp => !!fp && trusted.has(fp);
const recvPin = () => localStorage.recvPin || '';        // 我的接收码(空=关):别人私聊发我文件需先对码
const pinCache = new Map();                               // fp -> 我为对方输过的接收码(会话内记住,免重复输)
// 应用内 PIN 输入弹层(异步,不用阻塞式 prompt):resolve 为输入的码或 null(取消)
let pinResolve = null;
function askPin(name) {
  return new Promise(res => {
    pinResolve = res;
    document.getElementById('pin-title').textContent = t('enter_pin', { name });
    const inp = document.getElementById('pin-input'); inp.value = '';
    document.getElementById('pinmask').classList.add('show');
    setTimeout(() => inp.focus(), 50);
  });
}
function closePin(val) {
  document.getElementById('pinmask').classList.remove('show');
  const r = pinResolve; pinResolve = null; if (r) r(val);
}
{ const ok = document.getElementById('pin-ok'), cancel = document.getElementById('pin-cancel'), inp = document.getElementById('pin-input');
  if (ok) ok.onclick = () => closePin(inp.value.replace(/\D/g, '').slice(0, 6));
  if (cancel) cancel.onclick = () => closePin(null);
  if (inp) inp.addEventListener('keydown', e => { if (e.key === 'Enter') ok.click(); });
  const pm = document.getElementById('pinmask');
  if (pm) pm.onclick = e => { if (e.target.id === 'pinmask') closePin(null); };
}
function toggleTrust(fp) {
  if (!fp) return;
  if (trusted.has(fp)) trusted.delete(fp); else trusted.add(fp);
  localStorage.trustedFps = JSON.stringify([...trusted]);
  renderPeers();
}
...
// 友好名(仿 LocalSend):形容词 + 动物/水果,从 id 派生→稳定、好记、同型号也不撞名。
// 旧版按设备类型命名(两台 Mac 都叫"Mac"必撞),已弃用。
// 多语言:名字随设备自己的界面语言生成——中文界面得中文名(呆萌水獭),其余语言得英文名。
// 每台设备把生成好的名字字符串发给别人,所以花名册里各设备显示为它自己语言的名字(自然、诚实)。
const NAMES = {
  en: {
    sep: ' ',
    adj: ['Adorable','Brave','Bright','Calm','Clever','Cozy','Daring','Eager','Fancy','Fluffy','Gentle','Happy','Jolly','Keen','Kind','Lively','Lucky','Merry','Mighty','Nimble','Noble','Perky','Plucky','Proud','Quick','Quiet','Rapid','Shiny','Sleek','Snappy','Sunny','Swift','Witty','Zippy','Bold','Breezy','Chirpy','Comfy','Dapper','Frosty','Golden','Jazzy','Mellow','Peppy','Rosy','Silky','Spry','Wise'],
    noun: ['Avocado','Badger','Bunny','Cat','Cherry','Dolphin','Eagle','Falcon','Ferret','Fox','Gecko','Hedgehog','Iguana','Jaguar','Kiwi','Koala','Lemur','Lychee','Mango','Maple','Marmot','Narwhal','Newt','Otter','Owl','Panda','Peach','Penguin','Plum','Puffin','Quokka','Rabbit','Raccoon','Robin','Seal','Sparrow','Squid','Tiger','Turtle','Urchin','Viper','Walnut','Walrus','Willow','Wombat','Yak','Zebra','Lark'],
  },
  zh: {
    sep: '',
    adj: ['呆萌','勇敢','机灵','威武','软糯','圆滚','神气','活泼','温柔','精神','淘气','憨厚','灵动','迅捷','悠闲','俏皮','稳重','灵巧','欢快','慵懒','敏捷','傲娇','天真','优雅','沉稳','机敏','爽朗','甜美','潇洒','乖巧','灵秀','矫健','娇俏','憨萌','飒爽','明媚','轻盈','从容','俊俏','皎洁','灿烂','蓬勃','恬静','澄澈','温润','睿智','灵犀','磊落'],
    noun: ['熊猫','水獭','狐狸','老虎','桃子','企鹅','海豚','老鹰','雪貂','刺猬','猕猴','考拉','荔枝','芒果','松鼠','海豹','兔子','浣熊','麻雀','海龟','树懒','鹦鹉','锦鲤','河豚','袋鼠','银杏','柚子','柠檬','鲸鱼','灰狼','白鹭','蝴蝶','山猫','花鹿','雪狐','海星','犀鸟','蜂鸟','云雀','斑马','羊驼','貂熊','樱桃','葡萄','蜜獾','萤火','竹鼠','猫头'],
  },
};
function defaultName(id) {
  const p = (typeof LANG === 'string' && LANG.startsWith('zh')) ? NAMES.zh : NAMES.en;
  return p.adj[hashId(id) % p.adj.length] + p.sep + p.noun[hashId('n' + id) % p.noun.length];
}
// 迁移:旧版把"设备类型自动名"(Mac / iPhone·K2 等)写进了 localStorage,导致同型号撞名且改不掉。
// 清掉这类自动名,让友好名接管;真正手动起的名字(不匹配此模式)保留。
if (localStorage.deviceName && /^(iPhone|iPad|Mac|Windows|Android|Web)(·[A-Za-z0-9]{1,3})?$/.test(localStorage.deviceName))
  delete localStorage.deviceName;
let myName = localStorage.deviceName || defaultName(myId);   // 默认名不再落库,只有手动改名才存(见身份面板)
...
{ const st = document.getElementById('conv-star');   // 私聊头部"信任"星:切换当前对方设备的信任
  if (st) st.onclick = () => { const p = peers.get(currentConv); if (p && p.fp) { toggleTrust(p.fp); updateTitle(); } }; }
...
function openIdentity() {
  ...
  document.getElementById('id-pin').value = recvPin();
...
document.getElementById('id-save').onclick = () => {
  ...
  const pin = document.getElementById('id-pin').value.replace(/\D/g, '').slice(0, 6);
  if (pin) localStorage.recvPin = pin; else delete localStorage.recvPin;   // 存/清接收码
  myHue = pendingHue;
...
function updateTitle() {
  ...
  const star = document.getElementById('conv-star');
  if (currentConv === 'all') {
    ...
    if (star) {                                    // 私聊:显示"信任"星,已信任=点亮
      star.style.display = p && p.fp ? 'flex' : 'none';
      star.classList.toggle('on', p && isTrusted(p.fp));
      star.title = t(p && isTrusted(p.fp) ? 'trusted' : 'trust_device');
    }
  }
}
function renderConv() {
...
// 收到即自动落到"下载"文件夹(仿 LocalSend)。iOS Safari 不支持 blob 下载(会盖页面)→ 那里退回手动。
// 每个 fileId 只自动存一次(避免历史重绘时重复下载)。
const isIOS = /iP(hone|ad|od)/.test(navigator.userAgent);
const autoSavedIds = new Set();
function autoSave(blob, name, fileId) {
  if (isIOS || autoSavedIds.has(fileId)) return false;
  autoSavedIds.add(fileId);
  try {
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = name || 'file';
...
function fileCard(meta, role) {
  ...
  const api = {
    ...
    // 已收到:默认自动落到"下载"文件夹(仿 LocalSend);iOS 退回手动。图片仍内联预览。
    // isNew=首次到达(触发自动保存);历史重绘时不重复下载。
    saved(blob, isNew) {
      const auto = isNew ? autoSave(blob, meta.name, meta.fileId) : autoSavedIds.has(meta.fileId);
      if (isImg(meta.name)) {
        const url = URL.createObjectURL(blob);
        const wrap = row.querySelector('.wrap');
        wrap.innerHTML = `<div class="dev">${esc(meta.from || '')}</div>
          <div class="bubble img"><img src="${url}"></div>
          <div class="fc-imgact"><a class="fc-save">${t(auto ? 'save_as' : 'save')}</a></div>`;
        wrap.querySelector('.fc-save').onclick = () => saveBlob(blob, meta.name);
      } else {
        sizeEl.innerHTML = `<span class="ok">${t(auto ? 'saved_to' : 'received')} · ${fmtSize(meta.size)}</span>`;
        act.innerHTML = `<button class="fc-btn go">${t(auto ? 'save_as' : 'save')}</button>`;
...
function connect() {
  ...
  ws.onopen = () => ws.send(JSON.stringify({ type: 'hello', id: myId, name: myName, fp: myFp,
...
function setupDC(p, dc, id) {
  ...
  dc.onmessage = ev => {
    if (typeof ev.data === 'string') {
      ...
      } else if (m.t === 'freq') {          // 私聊:对方请求发文件给我 → 信任/无码=秒批;首次要码=pinreq;码错=pinbad
        const ok = isTrusted(p.fp) || !recvPin() || m.pin === recvPin();
        const reply = ok ? 'facc' : (m.pin == null ? 'pinreq' : 'pinbad');
        try { p.dc.send(JSON.stringify({ t: reply, fileId: m.fileId })); } catch {}
        // 接受后:实际接收卡在 fmeta 到达时创建(沿用现有逻辑),此处无需额外动作
      } else if (m.t === 'facc') {          // 对方已接受我的发送请求 → 开始推流
        const info = myFiles.get(m.fileId); if (!info) return;
        streamFileTo({ id, name: p.name, ua: p.ua }, info.file, m.fileId, info.card);
      } else if (m.t === 'pinreq') {        // 对方首次要接收码 → 应用内弹层输入,记住并重发(仅提示一次)
        const info = myFiles.get(m.fileId); if (!info) return;
        askPin(p.name).then(pin => {
          if (!pin) { info.card.unavailable(); return; }
          pinCache.set(p.fp, pin); sendFreq(p, m.fileId, pin);
        });
      } else if (m.t === 'pinbad') {        // 我给的接收码错 → 本次失败,清缓存,重发再试(不再无限追问)
        const info = myFiles.get(m.fileId); if (!info) return;
        pinCache.delete(p.fp); info.card.unavailable(); sysLine(t('pin_wrong', { name: p.name }));
...
async function sendFile(file) {
  ...
  } else {                                  // 私聊:先请求接收(默认对方秒批;对方设了接收码且我非信任→需先对码)
    const p = peers.get(conv);
    myFiles.set(fileId, { file, card, meta, group: false, conv });
    if (p && p.dc && p.dc.readyState === 'open') { card.sending(); sendFreq(p, fileId); }
    else { card.unavailable(); sysLine(t('no_direct', { name: p ? p.name : '' })); }
  }
}
// 私聊发送请求:带上文件元数据 + 我的指纹 + (可选)对方接收码;对方据此秒批或要码
function sendFreq(p, fileId, pin) {
  const info = myFiles.get(fileId); if (!info || !p.dc || p.dc.readyState !== 'open') return;
  const m = info.meta;
  p.dc.send(JSON.stringify({ t: 'freq', fileId, name: m.name, size: m.size, mime: m.mime, thumb: m.thumb,
    fromId: myId, from: myName, fromKind: myKind, fp: myFp,
