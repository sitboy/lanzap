...
- 📁 **폴더 통째로, 구조 그대로** — 끌어다 놓거나 선택하면 됩니다. 데스크톱 Chromium에서는 실제 폴더 트리로 디스크에 그대로 복원되고, 다른 브라우저에서는 ZIP으로 받습니다
...
두 단계: 같은 네트워크의 자동 검색에는 STUN만 제공되어 파일이 서버를 거치지 않으며, QR 코드로 명시적으로 페어링한 경우에만 최후의 수단으로 TURN 중계가 추가됩니다. 중계를 사용하면 UI에 **중계 경유**로 표시됩니다.
동작합니다. 다른 네트워크에서 링크를 열어도 모두 같은 방에 들어와 서로를 볼 수는 있지만
직접 연결은 되지 않으며, 이 경우 무한 로딩 대신 약 10초 후 UI에 명확히 안내됩니다.

폴더 **받기**는 모든 브라우저에서 되지만, **보내기**에는 폴더 선택 창이 필요합니다. iOS Safari에는 없어서 그곳에서는 이 입구를 숨깁니다. 폴더당 2000개 파일이 상한이며, 넘으면 일부를 조용히 빠뜨리는 대신 직접 압축하라고 분명히 알려 줍니다.
...
폴더는 보내기 전에 **묶지 않습니다**. 평범한 파일 묶음으로 전송되며 각 파일이 루트 기준 상대 경로를 함께 지니므로, 파일별 진행률·배압·재연결이 그대로 동작합니다. 플랫폼에 따라 갈라지는 것은 마지막 단계인 디스크 기록뿐입니다. File System Access는 트리를 곧바로 씁니다(스트리밍, 메모리에 쌓지 않음). 그 외에는 `public/zip.js`(약 130줄, 의존성 없음)가 무압축 ZIP으로 모읍니다. 들어오는 경로는 적대적 입력으로 간주해 정리하므로 대상 폴더 밖으로 나갈 수 없고, 디스크가 네트워크보다 느리면 받는 쪽이 보내는 쪽의 속도를 늦춰 밀린 데이터를 메모리에 쌓지 않습니다.
