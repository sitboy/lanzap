...
- 📁 **ทั้งโฟลเดอร์ โครงสร้างครบ** — ลากเข้ามาหรือเลือกก็ได้ บน Chromium บนคอมพิวเตอร์จะถูกเขียนกลับลงดิสก์เป็นโฟลเดอร์จริง ส่วนเบราว์เซอร์อื่นจะได้ไฟล์ ZIP
...
มีสองระดับ: การค้นหาอัตโนมัติในเครือข่ายเดียวกันจะได้รับเฉพาะ STUN (ไฟล์ไม่เคยผ่านเซิร์ฟเวอร์) ส่วนการจับคู่อย่างชัดเจนด้วยคิวอาร์โค้ดจะได้รีเลย์ TURN เพิ่มเป็นทางเลือกสุดท้าย และจะแสดงบนหน้าจอว่า **ผ่านรีเลย์**
...
ทุกเบราว์เซอร์**รับ**โฟลเดอร์ได้ แต่การ**ส่ง**ต้องใช้ตัวเลือกโฟลเดอร์ของระบบ ซึ่ง Safari บน iOS ไม่มี จึงซ่อนทางเข้านี้ไว้ที่นั่น ต่อโฟลเดอร์จำกัดที่ 2000 ไฟล์ หากเกินจะบอกให้คุณบีบอัดเองอย่างชัดเจน แทนที่จะเงียบ ๆ ตัดบางส่วนทิ้ง
...
โฟลเดอร์**ไม่ได้**ถูกบีบอัดก่อนส่ง แต่เดินทางเป็นชุดไฟล์ธรรมดา โดยแต่ละไฟล์พ่วงเส้นทางที่อ้างอิงจากรากโฟลเดอร์มาด้วย ความคืบหน้ารายไฟล์ การคุมแรงดันย้อนกลับ และการเชื่อมต่อใหม่จึงทำงานได้เหมือนเดิมทั้งหมด มีเพียงขั้นสุดท้ายคือการเขียนลงดิสก์เท่านั้นที่แยกตามแพลตฟอร์ม: File System Access เขียนโครงสร้างลงไปตรง ๆ (แบบสตรีม ไม่ค้างในหน่วยความจำ) ส่วนที่เหลือจะถูก `public/zip.js` (~130 บรรทัด ไม่มี dependency) รวบเป็น ZIP แบบไม่บีบอัด เส้นทางที่รับเข้ามาถือเป็นข้อมูลที่ไม่น่าไว้ใจและถูกทำความสะอาดเสมอ จึงหลุดออกนอกโฟลเดอร์ปลายทางไม่ได้ และเมื่อดิสก์ช้ากว่าเครือข่าย ฝั่งรับจะหน่วงฝั่งส่งแทนที่จะกองงานค้างไว้ใน RAM
